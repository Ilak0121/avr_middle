...
void CDS_light(unsigned int degree, unsigned int on)
{
   ...
   {
      // speed?? ???? ??? ????
      OCR3A=(degree*FREQ_CLKIO/2/PRESCALE/1000*DC_PERIOD)/level;
      PORTE|=0x01;   
   }
}
void buzzer(){
    PORTE |= 0x04; //PE2 is for buzzer  0000 0100 
    us_delay(10);
    PORTE &= 0xfb;
    us_delay(10);
}
int main(void)
{
    int degree,danger,danger_buffer=0;
	int clear_state;
    int temp_buffer;

    BIOS();                                                                                 //initial setting ADC0:CDS / ADC1:temp
    temp=25;
    temp_buffer=temp;
    while(1){
        /*--------------------------------------------------*/
       if(cds>=700)degree = 50;
       else if(cds>=600)degree = 80;
       else if(cds>=500)degree = 120;
       else if(cds>=400)degree = 160;
       else degree = 200;

       CDS_light(degree,0);                                                                 //PE1 is for front light 
        /*----------------------------------------------------*/
    	can_rx(2,&msg1);
       ...
       /*--------------------------------------------------*/
		if(danger==2){
			...
			LCD_Write(0,0,strBuff1);
			PORTE |= 0x08;                                                                    //alarm LED PE3	
		 }
       /*--------------------------------------------------*/
		if(!(temp-temp_buffer>7 || temp-temp_buffer>7)) temp_buffer=temp;                   
        else temp_buffer = (temp_buffer+temp)/2;
